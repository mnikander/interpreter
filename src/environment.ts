...
export function lookup(identifier: ASTAtom, environment: Environment): undefined | Symbol {
    const entry: undefined | Symbol = environment.symbols.get(String(identifier.value));
    if (entry !== undefined) {
        return entry;
    }
    else {
...
export const builtin: Environment = {
    parent: undefined,
...
function help(): string {
    let message: string = "\nSymbol\tUsage\t\tName\n------------------------------------------------\n";
    for (const [key, value] of builtin.symbols.entries()) {
