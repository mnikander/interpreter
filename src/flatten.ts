...
import { _Expression, _Block, _LetBind, is_block, is_boolean, is_call, is_binding, is_identifier, is_lambda, is_let, is_if, is_number, is_string, validate } from "./ast";
...
function flatten_nodes(nested_ast: _Expression, flat_ast: Flat_AST): Flat_AST {
    if (is_block(nested_ast)) {
        const index               = validate(nested_ast.id);
        const body: Id            = { id: validate(nested_ast.body.id) };
        let node: Flat_Block      = { id: index, token: nested_ast.token, tag: 'Flat_Block', body: body };
        flat_ast[index]           = node;
        flat_ast                  = flatten_nodes(nested_ast.body, flat_ast);
    }
    else if (is_lambda(nested_ast)) {
        const index               = validate(nested_ast.id);
        const binding_id          = { id: validate(nested_ast.binding.id) };
        const body_id             = { id: validate(nested_ast.body.id) };
        ...
        const index               = validate(nested_ast.id);
        const binding_id          = { id: validate(nested_ast.binding.id) };
        const value_id            = { id: validate(nested_ast.value.id) };
        const body_id             = { id: validate(nested_ast.body.id) };
        ...
        const index               = validate(nested_ast.id);
        const condition           = { id: validate(nested_ast.condition.id) }; 
        const then_branch         = { id: validate(nested_ast.then_branch.id) };
        const else_branch         = { id: validate(nested_ast.else_branch.id) };
        ...
        const index               = validate(nested_ast.id);
        const body_id             = { id: validate(nested_ast.fn.id) };
        const arg_id              = { id: validate(nested_ast.arg.id) };
